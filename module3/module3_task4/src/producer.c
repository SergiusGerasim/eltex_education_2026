...
int producer_run(const producer_config_t *config) {
    ...
    if (!ipc_semaphore_create(sem_key, &sem_id, &sem_created)) goto cleanup;
    if (!ipc_shared_memory_create(shm_key, config->segment_size, &shm_id, &shm_created)) goto cleanup;
    if (sem_created != shm_created) goto cleanup;
    if (!sem_created) goto cleanup;
...
cleanup:
    ...
    if (shm_created) ipc_shared_memory_remove(shm_id);
    if (sem_created) ipc_semaphore_remove(sem_id);
