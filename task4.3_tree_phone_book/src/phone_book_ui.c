...
static void print_contacts_in_order(const phone_book_node_t *node){
    if (node == NULL) return;

    print_contacts_in_order(node->left);
    print_contact(node);
    print_contacts_in_order(node->right);
}
...
static void add_contact(phone_book_node_t **head, phone_book_id_manager_t *id_manager){
    ...
    unsigned int new_contact_id = id_manager->free_ids != NULL
        ? id_manager->free_ids->id
        : id_manager->next_id;
