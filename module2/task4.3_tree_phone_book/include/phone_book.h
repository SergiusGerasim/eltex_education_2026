...
typedef struct phone_book_node{
    ...
    struct phone_book_node *left;
    struct phone_book_node *right;

    uint8_t height; // высота поддерева с корнем в данном узле
    // h < 1.44*log_2(n+2), при n = 10^9, высота дерева не превысит 44.
} phone_book_node_t;
