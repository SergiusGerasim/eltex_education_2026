...
static uint8_t height(phone_book_node_t *node){
    return node ? node->height : 0;
}

static int bfactor(phone_book_node_t *node){
    return height(node->right) - height(node->left);
}

static void fix_height(phone_book_node_t *node){
    uint8_t hl = height(node->left);
    uint8_t hr = height(node->right);
    node->height = (hl > hr ? hl : hr) + 1;
}

static phone_book_node_t *right_rotate(phone_book_node_t *node){
    phone_book_node_t *tmp = node->left;
    node->left = tmp->right;
    tmp->right = node;
    fix_height(node);
    fix_height(tmp);
...
static phone_book_node_t *left_rotate(phone_book_node_t *node){
    phone_book_node_t *tmp = node->right;
    node->right = tmp->left;
    tmp->left = node;
    fix_height(node);
    fix_height(tmp);
...
static phone_book_node_t *balance(phone_book_node_t *node){
    fix_height(node);
    if (bfactor(node) == 2){
...
static phone_book_node_t *phone_book_extract_node(phone_book_node_t *node,
    unsigned int id, phone_book_node_t **extracted);
...
static int phone_book_add(phone_book_node_t **head, phone_book_node_t *new_node){
    ...
    if (strcmp(temp_node->last_name, new_node->last_name) > 0){
        result = phone_book_add(&temp_node->left,new_node);
    }
    else{
        result = phone_book_add(&temp_node->right,new_node);
    }
...
phone_book_node_t *phone_book_find_by_id(
    phone_book_node_t *head,
    ...
    phone_book_node_t *found = phone_book_find_by_id(head->left, id);
    if (found != NULL) {
        return found;
    }

    return phone_book_find_by_id(head->right, id);
}
...
int phone_book_edit_last_name(phone_book_node_t **head, unsigned int id, const char *last_name){
    ...
    char new_last_name[NAMESIZE];
    if (copy_string(new_last_name, sizeof(new_last_name), last_name) != 0) return -1;

    phone_book_node_t *node = phone_book_find_by_id(*head, id);
    if (node == NULL) return -1;

    phone_book_node_t *extracted = NULL;
    *head = phone_book_extract_node(*head, id, &extracted);
    if (extracted == NULL) return -1;

    copy_string(extracted->last_name, sizeof(extracted->last_name), new_last_name);
    extracted->left = NULL;
    extracted->right = NULL;
    extracted->height = 1;

    return phone_book_add(head, extracted);
}
...
static void phone_book_clear_nodes(phone_book_node_t *node){
    if (node == NULL) return;

    phone_book_clear_nodes(node->left);
    phone_book_clear_nodes(node->right);
    free(node);
}

void phone_book_clear(phone_book_node_t **head, phone_book_id_manager_t *id_manager){
    if (head != NULL) {
        phone_book_clear_nodes(*head);
        *head = NULL;
    }
...
static phone_book_node_t *phone_book_find_min(phone_book_node_t *node){
    return node->left == NULL ? node : phone_book_find_min(node->left);
}

static phone_book_node_t *phone_book_remove_min(phone_book_node_t *node){
    if (node->left == NULL) return node->right;

    node->left = phone_book_remove_min(node->left);
    return balance(node);
}
...
static phone_book_node_t *phone_book_extract_node(phone_book_node_t *node,
    ...
    if (node->id == id) {
        phone_book_node_t *left = node->left;
        phone_book_node_t *right = node->right;
        node->left = NULL;
        node->right = NULL;
        node->height = 1;
        *extracted = node;
        ...
        phone_book_node_t *min = phone_book_find_min(right);
        min->right = phone_book_remove_min(right);
        min->left = left;
        return balance(min);
    }

    node->left = phone_book_extract_node(node->left, id, extracted);
    if (*extracted == NULL) {
        node->right = phone_book_extract_node(node->right, id, extracted);
    }
    return balance(node);
}
...
int phone_book_remove(phone_book_node_t **head, unsigned int id,
    phone_book_id_manager_t *id_manager){
    if (head == NULL || id_manager == NULL || id == 0) return -1;

    if (phone_book_find_by_id(*head, id) == NULL) return -1;
    if (phone_book_release_id(id_manager, id) != 0) return -1;

    phone_book_node_t *removed = NULL;
    *head = phone_book_extract_node(*head, id, &removed);
    free(removed);
    return 0;
}

static int phone_book_find_in_tree(phone_book_node_t *node, const char *name,
    const char *last_name, phone_book_search_ans_t *ans){
    if (node == NULL) return 0;

    if (phone_book_find_in_tree(node->left, name, last_name, ans) != 0) {
        return -1;
    }

    int name_matches = name == NULL || strcmp(node->name, name) == 0;
    int last_name_matches = last_name == NULL || strcmp(node->last_name, last_name) == 0;

    if (name_matches && last_name_matches) {
        phone_book_node_t **new_items =
            realloc(ans->items, (ans->count + 1) * sizeof(*ans->items));

        if (new_items == NULL) return -1;

        ans->items = new_items;
        ans->items[ans->count] = node;
        ans->count++;
    }

    return phone_book_find_in_tree(node->right, name, last_name, ans);
}
...
static phone_book_search_ans_t phone_book_find(phone_book_node_t *head, const char *name,
    ...
    if (name == NULL && last_name == NULL) return ans;

    if (phone_book_find_in_tree(head, name, last_name, &ans) != 0) {
        free(ans.items);
        ans.items = NULL;
        ans.count = 0;
    }
