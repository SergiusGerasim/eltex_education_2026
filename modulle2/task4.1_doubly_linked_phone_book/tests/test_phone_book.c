...
void test_create_and_add_should_reject_invalid_arguments(void){
    TEST_ASSERT_EQUAL_INT(-1, phone_book_node_create_and_add(
        NULL, "Герасимов", NULL, NULL, NULL, NULL, NULL, &head, &id_manager, NULL
    ));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_node_create_and_add(
        "Сергей", NULL, NULL, NULL, NULL, NULL, NULL, &head, &id_manager, NULL
    ));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_node_create_and_add(
        "Сергей", "Герасимов", NULL, NULL, NULL, NULL, NULL, NULL, &id_manager, NULL
    ));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_node_create_and_add(
        "Сергей", "Герасимов", NULL, NULL, NULL, NULL, NULL, &head, NULL, NULL
    ));
...
void test_create_and_add_should_return_created_id(void){
    unsigned int created_id = 0;

    TEST_ASSERT_EQUAL_INT(0, phone_book_node_create_and_add(
        "Сергей", "Герасимов", NULL, NULL, NULL, NULL, NULL,
        &head, &id_manager, &created_id
    ));

    TEST_ASSERT_EQUAL_UINT(1, created_id);
    TEST_ASSERT_EQUAL_UINT(created_id, head->id);
}

void test_add_should_sort_nodes_by_last_name_and_set_links(void){
    TEST_ASSERT_EQUAL_INT(0, add_contact("Третий", "Власов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Первый", "Абрамов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Второй", "Белов"));

    TEST_ASSERT_EQUAL_STRING("Абрамов", head->last_name);
    TEST_ASSERT_EQUAL_STRING("Белов", head->next->last_name);
    TEST_ASSERT_EQUAL_STRING("Власов", head->next->next->last_name);
    TEST_ASSERT_NULL(head->previous);
    TEST_ASSERT_EQUAL_PTR(head, head->next->previous);
    TEST_ASSERT_EQUAL_PTR(head->next, head->next->next->previous);
    TEST_ASSERT_NULL(head->next->next->next);
}

void test_add_should_keep_insertion_order_for_equal_last_names(void){
    TEST_ASSERT_EQUAL_INT(0, add_contact("Первый", "Белов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Второй", "Белов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Третий", "Белов"));
...
void test_remove_should_update_previous_links(void){
    TEST_ASSERT_EQUAL_INT(0, add_contact("Первый", "Абрамов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Второй", "Белов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Третий", "Власов"));
...
void test_edit_last_name_should_reinsert_node_in_sorted_position(void){
    TEST_ASSERT_EQUAL_INT(0, add_contact("Первый", "Абрамов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Второй", "Белов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Третий", "Власов"));

    TEST_ASSERT_EQUAL_INT(0, phone_book_edit_last_name(&head, 3, "Абалкин"));
    TEST_ASSERT_EQUAL_UINT(3, head->id);
    TEST_ASSERT_NULL(head->previous);
    TEST_ASSERT_EQUAL_UINT(1, head->next->id);
    TEST_ASSERT_EQUAL_PTR(head, head->next->previous);

    TEST_ASSERT_EQUAL_INT(0, phone_book_edit_last_name(&head, 3, "Яковлев"));
    TEST_ASSERT_EQUAL_UINT(1, head->id);
    TEST_ASSERT_NULL(head->previous);
    TEST_ASSERT_EQUAL_UINT(2, head->next->id);
    TEST_ASSERT_EQUAL_UINT(3, head->next->next->id);
...
void test_edit_last_name_should_keep_list_unchanged_on_error(void){
    char too_long[NAMESIZE + 1];
    make_string(too_long, NAMESIZE, 'A');

    TEST_ASSERT_EQUAL_INT(0, add_contact("Первый", "Абрамов"));
    TEST_ASSERT_EQUAL_INT(0, add_contact("Второй", "Белов"));
    phone_book_node_t *second = head->next;

    TEST_ASSERT_EQUAL_INT(-1, phone_book_edit_last_name(&head, 2, NULL));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_edit_last_name(&head, 2, too_long));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_edit_last_name(&head, 100, "Власов"));
    TEST_ASSERT_EQUAL_INT(-1, phone_book_edit_last_name(NULL, 2, "Власов"));

    TEST_ASSERT_EQUAL_STRING("Абрамов", head->last_name);
    TEST_ASSERT_EQUAL_STRING("Белов", second->last_name);
...
int main(void){
    ...
    RUN_TEST(test_create_and_add_should_return_created_id);
    RUN_TEST(test_add_should_sort_nodes_by_last_name_and_set_links);
    RUN_TEST(test_add_should_keep_insertion_order_for_equal_last_names);
    ...
    RUN_TEST(test_remove_should_update_previous_links);
    RUN_TEST(test_edit_last_name_should_reinsert_node_in_sorted_position);
    RUN_TEST(test_edit_last_name_should_keep_list_unchanged_on_error);
