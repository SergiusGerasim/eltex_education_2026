...
static int phone_book_add(phone_book_node_t **head, phone_book_node_t *new_node){
    ...
    while (current != NULL && strcmp(current->last_name, new_node->last_name) <= 0) {
...
static void phone_book_detach(phone_book_node_t **head, phone_book_node_t *node){
...
int phone_book_node_create_and_add(
    ...
    if (created_id != NULL) *created_id = new_node->id;
...
int phone_book_edit_last_name(phone_book_node_t **head, unsigned int id, const char *last_name){
    ...
    phone_book_detach(head, node);
    copy_string(node->last_name, sizeof(node->last_name), new_last_name);

    return phone_book_add(head, node);
}
...
int phone_book_remove(phone_book_node_t **head, unsigned int id,
    ...
    while (current != NULL){
        if (current->id == id){
            ...
            phone_book_detach(head, current);
            free(current);
            return 0;
        }
        current = current->next;
